...
public class WordBSTDictionary<K extends Comparable<? super K>, V> 
    ...
    @Override
    public V  get(K k) {
	if (this.root == null || k == null) return null;
    ...
    private V  get(BSTElement<K, V> node, K k) {
        ...
        if(key.compareTo(k) == 0) {
            ...
            BSTElement<K, V> right = node.getRight();
            ...
            BSTElement<K, V> left = node.getLeft();
    ...
    private ArrayList<K> inorder(BSTElement<K, V> node, ArrayList<K> list) {
        if (node == null){
            return null;
        }
