...
public class FreqWordMaxBTree<K extends Integer, V extends String> 
    ...
    private void insert(BSTElement<K, V> node, 
        ...
        if (freq.compareTo(node.getKey()) == 0)
            ...
            if(node.getLeft() == null){
                BSTElement<K, V> newNode = new BSTElement();
            ...
            if(node.getLeft().getKey() != null && node.getRight().getKey() != null){
                if(node.getLeft().getKey() < node.getRight().getKey()){
                    insert(node.getLeft(), freq, word);
                }
                if(node.getLeft().getKey() > node.getRight().getKey()){
                    insert(node.getRight(), freq, word);
                }
            }            
        }
    }
	...
	AbstractMap.SimpleEntry<K, V> en; 
        ...
        if (node.getLeft() == null && node.getRight() == null){
            ...
            K k;
            V v;
            if (node.getLeft().getKey().
                    compareTo(node.getRight().getKey()) > 0){
